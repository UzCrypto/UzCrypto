...
const admins		=		[990008997, 319797568, 657884680, 295523254];
...
const bot			=		new Telegram(
	"1082384063:AAHBvyplLIB1ZGP5gLmkGia2P_zW0gI4dxk",
	{ polling: true }
);
...
let { status } = await bot.getChatMember(-1001227539743, message.from.id);
	if(status === 'left') return message.send(`➕ *Для того, чтобы* начать работу с ботом, *вы должны быть подписанным* на канал https://t.me/moneygotv и после этой нажать в любую кнопку`, {
		parse_mode: 'Markdown',
		reply_markup: {
			inline_keyboard: [
				[{ text: '➕ Подписаться', url: 'https://t.me/moneygotv' }]
			]
		}
	});	
